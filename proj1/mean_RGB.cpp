...
int main(int argc, char* argv[])
{
	...
	for (int index_rori = 0; index_rori < imageHeight; index_rori++) {
		for (int index_cori = 0; index_cori < imageWidth; index_cori++) {
				...
				rtemp = accumulate(pixr_wd.begin(), pixr_wd.end(),1);
				gtemp = accumulate(pixg_wd.begin(), pixg_wd.end(), 1);
				btemp = accumulate(pixb_wd.begin(), pixb_wd.end(), 1);

				pix[index_rori*imageWidth*BytesPerPixel + index_cori * BytesPerPixel] =  round(rtemp/(N*N));
				pix[index_rori*imageWidth*BytesPerPixel + index_cori * BytesPerPixel+1] = round(gtemp / (N*N));
				pix[index_rori*imageWidth*BytesPerPixel + index_cori * BytesPerPixel+2] = round(btemp / (N*N));
